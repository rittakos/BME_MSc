  ...
  override fun draw(vararg uniformProviders : UniformProvider) { 
    ...
    for(provider in uniformProviders){
      for(structName in provider.glslStructNames) {
        val descList = uniformDescriptors[structName] ?: continue
        for(uniformDesc in descList) {
          val descListAsString = descList.map{it.name}.joinToString(", ")
          val providedUniformsAsString = provider.uniforms.toList().map{it.first}.joinToString(", ")
          provider[uniformDesc.name]?.commit(gl, uniformDesc.location, textureUnitCount) ?:
           console.warn("Uniform ${uniformDesc.name} in struct ${structName} from active uniform list ${descListAsString} not found among provided uniforms ${providedUniformsAsString}")
